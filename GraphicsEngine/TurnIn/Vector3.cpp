...
Vector3::Vector3(const Vector3& Other) : Position{Other.GetX(), Other.GetY(), Other.GetZ(), Other.GetHomogeneous()} {}

Vector3::Vector3(Vector3&& Other) noexcept : Position{
	Other.GetX(), Other.GetY(), Other.GetZ(), Other.GetHomogeneous()
} {}
...
Vector3& Vector3::operator=(const Vector3& Other) {
	...
	SetY(Other.GetY());
	SetZ(Other.GetZ());
	SetHomogeneous(Other.GetHomogeneous());
	return *this;
}
...
Vector3 Vector3::operator/(const double Other) const {
	...
	_.SetHomogeneous(GetHomogeneous()/ Other);
	return _;
}
...
double Vector3::GetHomogeneous() const { return this->Position[3]; }
void Vector3::SetHomogeneous(const double NewW) { this->Position[3] = NewW; }
...
std::string Vector3::ToString() const {
	return Stringf("Vector2 {[X: %f, Y: %f, Z: %f] Homogenous: %f}", GetX(), GetY(), GetZ(), GetHomogeneous());
}
