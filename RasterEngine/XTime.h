...
// Use it for tracking time intervals in seconds with float percision.
...
class XTime
{
	...
	struct THREAD_DATA
	{
		...
		float totalTime, deltaTime, smoothDelta, blendWeight;
		float samplesPerSecond, lastSecond, actualHz;
...
public:
	...
	XTime(unsigned char samples = 10, float smoothFactor = 0.75);
	...
	float TotalTime(); // will be the same per frame
	// How Much time has passed exactly since the timer was restarted?(slower)
	float TotalTimeExact(); // will be different between calls
